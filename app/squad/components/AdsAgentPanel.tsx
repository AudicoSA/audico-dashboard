...
const MOCK_PERFORMANCE_DATA: PerformanceData[] = [
  { date: 'Mon', impressions: 45000, clicks: 1650, conversions: 78, cost: 1155, revenue: 6240, ctr: 3.67, roas: 5.4, cpa: 14.81 },
  { date: 'Tue', impressions: 48500, clicks: 1820, conversions: 89, cost: 1274, revenue: 7120, ctr: 3.75, roas: 5.59, cpa: 14.31 },
  { date: 'Wed', impressions: 52000, clicks: 1950, conversions: 94, cost: 1365, revenue: 7520, ctr: 3.75, roas: 5.51, cpa: 14.52 },
  { date: 'Thu', impressions: 49800, clicks: 1780, conversions: 85, cost: 1246, revenue: 6800, ctr: 3.57, roas: 5.46, cpa: 14.66 },
  { date: 'Fri', impressions: 54200, clicks: 2100, conversions: 102, cost: 1470, revenue: 8160, ctr: 3.87, roas: 5.55, cpa: 14.41 },
  { date: 'Sat', impressions: 41000, clicks: 1580, conversions: 76, cost: 1106, revenue: 6080, ctr: 3.85, roas: 5.50, cpa: 14.55 },
  { date: 'Today', impressions: 40710, clicks: 1522, conversions: 82, cost: 1255.50, revenue: 6560, ctr: 3.74, roas: 5.22, cpa: 15.31 }
]
...
export default function AdsAgentPanel() {
  ...
  const todayMetrics = {
    ...
    avgCpa: campaigns.filter(c => c.status === 'active' && c.conversions > 0).reduce((sum, c) => sum + c.cpa, 0) / campaigns.filter(c => c.status === 'active' && c.conversions > 0).length,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {budgetUtilization >= 90 && (
            <div className="flex items-center gap-2 px-3 py-1 bg-orange-500/20 border border-orange-500/30 rounded-lg">
              ...
              <span className="text-sm font-medium text-orange-400">Alert: Budget Limit Approaching</span>
            </div>
          )}
        </div>
        ...
        <div>
          <div className="mb-2 flex justify-between text-sm">
            ...
            <span className={`font-medium ${
              budgetUtilization >= 95 ? 'text-red-400' :
              budgetUtilization >= 80 ? 'text-orange-400' :
              'text-white'
          ...
          <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all ${
                budgetUtilization >= 95 ? 'bg-gradient-to-r from-red-500 to-orange-500' :
                budgetUtilization >= 80 ? 'bg-gradient-to-r from-orange-500 to-yellow-500' :
          ...
          <p className={`text-xs mt-2 font-medium ${
            ...
            {budgetUtilization < 80 
              ...
              : budgetUtilization < 95 
              ? '⚠️ Approaching limit - monitor closely'
              : '🚨 Near or exceeded limit - action required'}
          </p>
        </div>
      </motion.div>

      {/* Key Metrics with CTR, CPA, ROAS */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <MetricCard
          label="CTR"
          value={`${todayMetrics.avgCtr.toFixed(2)}%`}
          change={+5.8}
          icon={TrendingUp}
          color="cyan"
        />
        <MetricCard
          label="CPA"
          value={`R ${todayMetrics.avgCpa.toFixed(2)}`}
          change={-3.2}
          icon={DollarSign}
          color="orange"
          inverseGood={true}
        ...
        <MetricCard
          label="ROAS"
          value={`${todayMetrics.avgRoas.toFixed(1)}x`}
          change={+7.4}
          icon={Activity}
          color="green"
        />
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={performanceData}>
              ...
              />
              <Line type="monotone" dataKey="ctr" stroke="#06b6d4" strokeWidth={2} dot={{ fill: '#06b6d4' }} name="CTR" />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={performanceData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2c2c2c" />
              <XAxis dataKey="date" stroke="#666" style={{ fontSize: '12px' }} />
              <YAxis stroke="#666" style={{ fontSize: '12px' }} tickFormatter={(value) => `R${value}`} />
              <Tooltip 
                contentStyle={{ backgroundColor: '#1c1c1c', border: '1px solid #333', borderRadius: '8px' }}
                labelStyle={{ color: '#fff' }}
                formatter={(value: number | undefined) => value !== undefined ? `R ${value.toFixed(2)}` : 'N/A'}
              />
              <Line type="monotone" dataKey="cpa" stroke="#f59e0b" strokeWidth={2} dot={{ fill: '#f59e0b' }} name="CPA" />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={performanceData}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="#2c2c2c" />
              <XAxis dataKey="date" stroke="#666" style={{ fontSize: '12px' }} />
              <YAxis stroke="#666" style={{ fontSize: '12px' }} tickFormatter={(value) => `${value}x`} />
              <Tooltip 
                contentStyle={{ backgroundColor: '#1c1c1c', border: '1px solid #333', borderRadius: '8px' }}
                labelStyle={{ color: '#fff' }}
                formatter={(value: number | undefined) => value !== undefined ? `${value.toFixed(2)}x` : 'N/A'}
              />
              <Area type="monotone" dataKey="roas" stroke="#a3e635" fillOpacity={1} fill="url(#colorRoas)" name="ROAS" />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
      ...
      {/* AI Recommendations with Approval Interface */}
...
function MetricCard({ label, value, change, icon: Icon, color, inverseGood = false }: {
  label: string
  value: string
  change: number
  icon: any
    ...
    <div className="bg-[#252525] border border-white/5 rounded-xl p-4 hover:border-lime-500/30 transition-all">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-400">
            ...
            <span className={budgetPercent > 90 ? 'text-orange-400 font-medium' : 'text-gray-400'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-3">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-cyan-400">{campaign.ctr.toFixed(2)}%</p>
        </div>
        <div>
          <p className="text-xs text-gray-500">CPA</p>
          <p className="text-sm font-semibold text-orange-400">R {campaign.cpa.toFixed(2)}</p>
        </div>
        <div>
